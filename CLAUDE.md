...
- **`src/session.ts`** - `ClaudeSession` class wrapping Agent SDK V2 with adaptive thinking, effort levels (medium/high/max), streaming, session persistence (`/data/session.json`), and defense-in-depth safety checks
...
- **`src/jobs.ts`** - Shared cron job registry I/O (read, write, cancel, add — atomic writes)

### Handlers (`src/handlers/`)

Each message type has a dedicated async handler:
- **`commands.ts`** - `/start`, `/new`, `/stop`, `/status`, `/resume`, `/restart`, `/retry`, `/jobs`
...
- **`callback.ts`** - Inline keyboard button handling for ask_user MCP and cron job confirm/reject/cancel
- **`streaming.ts`** - Shared `StreamingState`, status callback factory, MCP request polling (ask_user + cron confirm)
...
- **`ask_user_mcp/`** - `ask_user` tool: presents options as Telegram inline keyboard buttons
- **`cron_mcp/`** - `register_cron_job` tool: validates job params, writes confirm request, user taps Confirm/Reject
...
- `/data/jobs/registry.json` - Cron job registry (shared with systemd runner)
- `/tmp/telegram-bot/` - Downloaded photos/documents (scratch, not persistent)
- `/tmp/cron-confirm-*.json` - Pending cron job confirmations (ephemeral, cleaned on startup)
